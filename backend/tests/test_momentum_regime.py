"""测试momentum_regime函数的参数问题"""

import sys
sys.path.insert(0, r'd:\期货自动进化因子挖掘系统\backend')

import pandas as pd
import numpy as np
from quant_engine.ops.gp_individual import GPIndividual, GPNode
from quant_engine.ops.gp_fitness import FitnessEvaluator

print("=== 测试momentum_regime函数的参数问题 ===")

# 创建测试数据
np.random.seed(42)
n = 100
data = pd.DataFrame({
    'open': np.random.randn(n) + 100,
    'high': np.random.randn(n) + 101,
    'low': np.random.randn(n) + 99,
    'close': np.random.randn(n) + 100,
    'volume': np.random.randint(1000, 10000, n),
    'open_interest': np.random.randint(1000, 10000, n),
    'near_close': np.random.randn(n) + 100,
    'far_close': np.random.randn(n) + 100,
    'days_to_expiry': np.random.randint(1, 30, n),
})

evaluator = FitnessEvaluator()

# 测试1: 正常参数
print("\n测试1: 正常参数 (fast=10, slow=30)")
node1 = GPNode(
    node_type='func',
    name='momentum_regime',
    children=[
        GPNode(node_type='var', name='close'),
        GPNode(node_type='const', name='const', value=10),
        GPNode(node_type='const', name='const', value=30),
    ]
)
ind1 = GPIndividual(root=node1, id="test_normal", generation=0)
result1 = evaluator.evaluate(ind1, data, symbol="TEST")
print(f"结果: valid={result1.valid}, sharpe={result1.sharpe:.4f}, error={result1.error_message}")

# 测试2: fast为负浮点数（数据库中出现的）
print("\n测试2: fast为负浮点数 (fast=-8.8377)")
node2 = GPNode(
    node_type='func',
    name='momentum_regime',
    children=[
        GPNode(node_type='var', name='close'),
        GPNode(node_type='const', name='const', value=-8.8377),
        GPNode(node_type='const', name='const', value=30),
    ]
)
ind2 = GPIndividual(root=node2, id="test_neg_float", generation=0)
result2 = evaluator.evaluate(ind2, data, symbol="TEST")
print(f"结果: valid={result2.valid}, sharpe={result2.sharpe:.4f}, error={result2.error_message}")

# 测试3: slow为负浮点数
print("\n测试3: slow为负浮点数 (slow=-8.8377)")
node3 = GPNode(
    node_type='func',
    name='momentum_regime',
    children=[
        GPNode(node_type='var', name='close'),
        GPNode(node_type='const', name='const', value=10),
        GPNode(node_type='const', name='const', value=-8.8377),
    ]
)
ind3 = GPIndividual(root=node3, id="test_slow_neg", generation=0)
result3 = evaluator.evaluate(ind3, data, symbol="TEST")
print(f"结果: valid={result3.valid}, sharpe={result3.sharpe:.4f}, error={result3.error_message}")

# 测试4: fast为浮点数（非负）
print("\n测试4: fast为浮点数 (fast=2.539)")
node4 = GPNode(
    node_type='func',
    name='momentum_regime',
    children=[
        GPNode(node_type='var', name='close'),
        GPNode(node_type='const', name='const', value=2.539),
        GPNode(node_type='const', name='const', value=30),
    ]
)
ind4 = GPIndividual(root=node4, id="test_float", generation=0)
result4 = evaluator.evaluate(ind4, data, symbol="TEST")
print(f"结果: valid={result4.valid}, sharpe={result4.sharpe:.4f}, error={result4.error_message}")

print("\n=== 测试完成 ===")
