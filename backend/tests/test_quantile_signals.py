...
# 测试几个不同的公式
formulas = [
    "(intraday_range(volume, close, high) - ts_max(open, 20))",
    "(intraday_range(volume, open, high) - ts_max(open, 20))",
    "20",
    "high",
    "ts_max(open, 20)",
]
...
# 计算各个组成部分
intraday1 = FACTOR_REGISTRY.get('intraday_range').func(data['volume'], data['close'], data['high'])
intraday2 = FACTOR_REGISTRY.get('intraday_range').func(data['volume'], data['open'], data['high'])
ts_max = FACTOR_REGISTRY.get('ts_max').func(data['open'], 20)

# 计算复合公式
formula_values = {
    "intraday1 - ts_max": intraday1 - ts_max,
    "intraday2 - ts_max": intraday2 - ts_max,
    "20": np.full(len(data), 20.0),
    "high": data['high'],
    "ts_max": ts_max,
}
...
for name, values in formula_values.items():
    normalized = evaluator._normalize_factor_quantile(values)
    valid = normalized[~np.isnan(normalized)]
    print(f"\n{name}:")
    print(f"  分位数归一化后: min={valid.min():.4f}, max={valid.max():.4f}, mean={valid.mean():.4f}, std={valid.std():.4f}")
...
for i in range(len(names)):
    for j in range(i+1, len(names)):
        ...
        norm1 = evaluator._normalize_factor_quantile(formula_values[name1])
        norm2 = evaluator._normalize_factor_quantile(formula_values[name2])
